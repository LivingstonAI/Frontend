...
export default function SnowAITradingSim() {
  ...
  const createInitialAgentState = (template) => ({
    ...
    model: (template.id === 11 || template.id === 12) ? null : new DoubleDQN(
      INPUT_SIZE, 
      ACTION_SIZE, 
      template.hiddenSize || HIDDEN_SIZE, 
      template.hiddenSize2 || HIDDEN_SIZE_2, 
      template.lr, 
      DISCOUNT_FACTOR
    ), 
    ...
    const interval = setInterval(() => {
      ...
      setAgents(prevAgents => prevAgents.map(agent => {
        ...
        if (action === 0) {
            ...
            // Calculate reward for the SELL action
            if (agent.model) {
...
const INPUT_SIZE = 7; // Expanded state space
const ACTION_SIZE = 3; // 0: BUY, 1: HOLD, 2: SELL
const HIDDEN_SIZE = 32; // Increased from 8
const HIDDEN_SIZE_2 = 16; // Second hidden layer
...
const AGENT_TEMPLATES = [
  { 
    ...
    hiddenSize2: 16,
    risk: 'med',
    rewardType: 'balanced', // Rewards both P&L and position quality
    stateFeatures: 'standard', // Uses all 7 features
  ...
  { 
    ...
    hiddenSize2: 12,
    risk: 'high',
    rewardType: 'scalper', // Heavy reward for frequent small wins
    stateFeatures: 'short-term', // Focuses on recent price action
  ...
  { 
    ...
    hiddenSize2: 20,
    risk: 'low',
    rewardType: 'trend', // Rewards holding winning positions longer
    stateFeatures: 'long-term', // Emphasizes longer-term indicators
  ...
  { 
    ...
    hiddenSize2: 16,
    risk: 'med',
    rewardType: 'momentum', // Rewards riding momentum
    stateFeatures: 'momentum', // Focuses on acceleration signals
  ...
  { 
    ...
    hiddenSize2: 14,
    risk: 'high',
    rewardType: 'aggressive', // Maximum reward for big wins, accepts big losses
    stateFeatures: 'volatility', // Emphasizes volatility signals
  ...
  { 
    ...
    hiddenSize2: 24,
    risk: 'low',
    rewardType: 'conservative', // Heavy penalty for losses, modest reward for gains
    stateFeatures: 'risk-aware', // Includes drawdown signals
  ...
  { 
    ...
    hiddenSize2: 16,
    risk: 'high',
    rewardType: 'momentum', // Specialized momentum rewards
  ...
  { 
    ...
    hiddenSize2: 16,
    risk: 'med',
    rewardType: 'mean-reversion', // Rewards buying dips, selling peaks
    stateFeatures: 'mean-reversion', // Emphasizes SMA distance
  ...
  { 
    ...
    hiddenSize2: 14,
    risk: 'high',
    rewardType: 'volatility', // Specialized for chaotic conditions
  ...
  { 
    ...
    hiddenSize2: 16,
    risk: 'med',
    rewardType: 'contrarian', // Rewards contrarian plays
    stateFeatures: 'contrarian', // Inverted trend signals
...
/**
 * Improved DDQN with proper backpropagation and 3-layer architecture
 */
class DoubleDQN {
    ...
    _createNetwork(inputs, outputs, hidden1, hidden2) {
        ...
        return {
            w1: Array.from({ length: inputs }, () => 
                Array(hidden1).fill(0).map(() => xavier(inputs, hidden1))),
            b1: Array(hidden1).fill(0),
            w2: Array.from({ length: hidden1 }, () => 
                Array(hidden2).fill(0).map(() => xavier(hidden1, hidden2))),
            b2: Array(hidden2).fill(0),
            w3: Array.from({ length: hidden2 }, () => 
                Array(outputs).fill(0).map(() => xavier(hidden2, outputs))),
    ...
    train() {
        ...
        for (const [state, action, reward, nextState, done] of batch) {
            ...
            outputGrads[action] = error;

            // Update W3 (Hidden2 -> Output) and B3
            for (let i = 0; i < this.outputs; i++) {
                for (let j = 0; j < this.hidden2; j++) {
                    this.q_network.w3[j][i] += this.lr * outputGrads[i] * hidden2[j];
                }
                this.q_network.b3[i] += this.lr * outputGrads[i];
            }
            ...
            for (let j = 0; j < this.hidden2; j++) {
                ...
                // ReLU derivative: gradient flows only if hidden2[j] > 0
            ...
            for (let i = 0; i < this.hidden2; i++) {
                for (let j = 0; j < this.hidden1; j++) {
                    this.q_network.w2[j][i] += this.lr * hidden2Grads[i] * hidden1[j];
                }
                this.q_network.b2[i] += this.lr * hidden2Grads[i];
            }
            ...
            for (let i = 0; i < this.hidden1; i++) {
                for (let j = 0; j < this.inputs; j++) {
                    this.q_network.w1[j][i] += this.lr * hidden1Grads[i] * state[j];
                }
                this.q_network.b1[i] += this.lr * hidden1Grads[i];
            }
        }
...
// Enhanced state calculation with agent-specific feature weighting
const calculateInputs = (allCandles, currentPrice, agent) => {
    if (allCandles.length < 20) return Array(INPUT_SIZE).fill(0);
    ...
    smaDist = Math.max(-1, Math.min(1, smaDist * 10)); // Scale and clip
    
    // Feature 2: Volatility (normalized 0 to 1)
    ...
    if (stateType === 'volatility') {
        normVolatility *= 1.5; // Amplify for volatility-focused agents
    }
    normVolatility = Math.min(1, normVolatility);
    
    // Feature 3: Price Change (normalized -1 to 1)
    ...
    if (stateType === 'short-term' || stateType === 'scalper') {
        normPriceChange *= 1.5; // Amplify for scalpers
    }
    normPriceChange = Math.max(-1, Math.min(1, normPriceChange));
    
    // Feature 4: Trend Strength (-1 to 1)
    let trend = 0;
    if (sma5 && sma20 && sma50) {
        if (stateType === 'contrarian') {
            // Inverted for contrarian
    ...
    if (allCandles.length >= 10) {
        ...
        if (stateType === 'momentum') {
            momentum *= 1.5; // Amplify for momentum traders
        }
    }
    ...
    switch(rewardType) {
        case 'scalper':
            if (action === 2 && pnl > 0) {
                ...
                reward = (pnl / INITIAL_CASH * 100) * (holdTime < 10 ? 1.5 : 1.0);
            } else if (action === 0) {
                reward = -0.2; // Higher trade penalty
            } else if (action === 1 && agent.shares > 0) {
                reward = -0.05 * holdTime; // Penalty for holding too long
            }
            break;
        ...
        case 'trend':
            if (action === 2 && pnl > 0) {
                // Reward longer holds
                reward = (pnl / INITIAL_CASH * 100) * (1 + holdTime / 50);
            } else if (action === 1 && agent.shares > 0) {
                ...
                reward = -0.05; // Small trade penalty
            }
            break;
            
        case 'momentum':
            if (action === 0) {
                // Check if buying into momentum
                const sma5 = calculateSMA(allCandles, 5);
                const sma20 = calculateSMA(allCandles, 20);
                if (sma5 && sma20 && sma5 > sma20) {
                    reward = 0.1; // Reward buying momentum
                } else {
                    reward = -0.2; // Penalty for counter-momentum entry
                }
            } else if (action === 2 && pnl > 0) {
                reward = pnl / INITIAL_CASH * 100;
            }
            break;
        ...
        case 'mean-reversion':
            if (action === 0) {
                ...
                // Reward quick mean-reversion profits
                reward = (pnl / INITIAL_CASH * 100) * 1.5;
            }
            break;
            
        case 'aggressive':
            if (action === 2) {
                // Amplified rewards/penalties
                reward = (pnl / INITIAL_CASH * 100) * 2;
            } else if (action === 0) {
                reward = -0.1;
            }
            break;
        ...
        case 'conservative':
            if (action === 2 && pnl < 0) {
                ...
                // Modest reward for gains
                reward = (pnl / INITIAL_CASH * 100) * 0.5;
            } else if (action === 1 && agent.shares > 0) {
                // Reward stable positions
                reward = 0.02;
            }
            break;
        ...
        case 'volatility':
            ...
            if (volatility > avgVol * 1.2) {
                // High volatility - reward aggressive action
                if (action === 0 || (action === 2 && Math.abs(pnl) > INITIAL_CASH * 0.01)) {
                    reward = 0.2;
                }
            }
            if (action === 2) {
        ...
        case 'contrarian':
            if (action === 0) {
                // Reward buying when trend is down
                const sma5 = calculateSMA(allCandles, 5);
                const sma20 = calculateSMA(allCandles, 20);
                if (sma5 && sma20 && sma5 < sma20) {
                    reward = 0.15;
                ...
                reward = (pnl / INITIAL_CASH * 100) * 1.3;
            }
            break;
          ...
          {agent.model && (
            <div style={{marginBottom:'20px'}}>
              ...
              <div style={{backgroundColor: THEME.terminalBg, padding:'12px', borderRadius:'8px', fontFamily: 'monospace', fontSize:'12px', color: THEME.terminalText}}>
                Input Layer: {INPUT_SIZE} neurons<br/>
                ...
                Output Layer: {ACTION_SIZE} neurons (Linear)
              </div>
            </div>
          )}
