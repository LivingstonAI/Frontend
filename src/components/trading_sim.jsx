...
export default function SnowAITradingSim() {
  ...
  const [assetName, setAssetName] = useState('BTC'); // New state for asset name
  ...
  const calculateSMA = (data, period) => {
      ...
      const recent = data.slice(-period).map(d => d.c);
  ...
  const calculateVolatility = (data, period) => {
      if (data.length < period) return 0;
      const prices = data.slice(-period).map(d => d.c);
      const avg = prices.reduce((a, b) => a + b, 0) / period;
      const variance = prices.map(p => (p - avg) ** 2).reduce((a, b) => a + b, 0) / period;
      return Math.sqrt(variance) / avg; // Normalized volatility
  };

  const getInputs = (candles, currentPrice) => {
      // Need at least 21 candles for a 20-period SMA
      if (candles.length < 20) return [0, 0, 0, 0];

      const priceChange = (currentPrice - candles[candles.length - 2].c) / candles[candles.length - 2].c;
      const sma20 = calculateSMA(candles, 20);
      const smaDistance = (currentPrice - sma20) / sma20;
      const volatility = calculateVolatility(candles, 10);
      const trendChange = (sma20 - calculateSMA(candles.slice(0, -1), 20)) / sma20;

      // Normalize inputs (simple normalization for demo)
      const inputs = [
          Math.tanh(priceChange * 100), // Price Change (scaled)
          Math.tanh(smaDistance * 10), // SMA Distance (scaled)
          Math.min(1, volatility * 100), // Volatility (capped)
          Math.tanh(trendChange * 100) // Trend Change (scaled)
      ];

      return inputs;
  };

  const executeTrade = (agent, price, action, amount) => {
      const newAgent = { ...agent };
      const commissionAmount = price * amount * COMMISSION;
      ...
      if (action === 'BUY') {
          const cost = price * amount + commissionAmount;
          if (newAgent.cash >= cost) {
              newAgent.cash -= cost;
              newAgent.shares += amount;
              newAgent.history.push({ 
                  ...
                  sharesBefore: agent.shares
              });
              logMsg = `Bought ${amount.toFixed(4)} @ ${price.toFixed(2)}. Cost: ${cost.toFixed(2)}`;
              logType = 'success';
          } else {
              logMsg = "BUY signal ignored: Insufficient cash.";
              logType = 'warning';
          }
      } else if (action === 'SELL' && newAgent.shares > 0) {
          const revenue = price * amount - commissionAmount;
          const sellHistory = {
              t: timestamp,
              type: 'SELL',
              price: price,
          ...
          // Find the related buy trade (simplistic FIFO for demo)
          const buyTrade = newAgent.history.find(h => h.type === 'BUY' && !h.sold);
          
          let pnl = 0;
          if (buyTrade) {
              const boughtPrice = buyTrade.price;
              pnl = (price - boughtPrice) * amount - commissionAmount;
              sellHistory.pnl = pnl;
              buyTrade.sold = true; // Mark as covered (for simple FIFO/PNL tracking)
          }

          newAgent.cash += revenue;
          newAgent.shares -= amount;
          newAgent.history.push(sellHistory);

          logMsg = `Sold ${amount.toFixed(4)} @ ${price.toFixed(2)}. P&L: ${pnl.toFixed(2)}`;
          logType = pnl > 0 ? 'success' : 'danger';
      }

      newAgent.portfolioValue = newAgent.cash + newAgent.shares * price;
      newAgent.logs.push({ msg: logMsg, type: logType });
      newAgent.lastAction = action;

      // Log only the last 50 entries
      newAgent.logs = newAgent.logs.slice(-50);
      newAgent.equityCurve.push(newAgent.portfolioValue);
      newAgent.candles = agent.candles.slice(-10).concat(agent.candles.slice(-1).length ? [] : [agent.candles.slice(-1)[0]]); // Keep mini-chart up to date

      return newAgent;
  };

  // --- CORE AGENT UPDATE LOGIC ---
  ...
  const updateAgents = (currentCandle, allCandles) => {
      ...
      setAgents(prevAgents => prevAgents.map(agent => {
          ...
          let newAgent = { ...agent, candles: [...agent.candles.slice(-19), currentCandle] }; // Keep last 20 candles for mini-chart/indicators
          newAgent.prevValue = agent.portfolioValue; // Save previous value for training

          // 1. Benchmark (Buy & Hold) Logic
          if (newAgent.id === 11) {
              if (!newAgent.hasBoughtInitial) {
                  const amountToBuy = newAgent.cash * (1 - COMMISSION) / currentPrice;
                  newAgent = executeTrade(newAgent, currentPrice, 'BUY', amountToBuy);
          ...
          // 2. Buy The Dip Agent (Dip-Buyer) Logic
          if (newAgent.id === 12) {
              const sma5 = calculateSMA(newAgent.candles, 5);
              const dropPercent = (sma5 - currentPrice) / sma5 * 100;

              if (!newAgent.hasBoughtInitial) {
                  // Wait for first 5 candles to establish SMA
                  if (newAgent.candles.length >= 5) {
                      newAgent.hasBoughtInitial = true;
                  }
              }

              if (newAgent.hasBoughtInitial) {
                  // Buy Dip (2% drop below 5-period SMA)
                  if (dropPercent >= 2 && newAgent.cash > 0) {
                      const amountToBuy = (newAgent.cash * 0.5) * (1 - COMMISSION) / currentPrice; // Use 50% of remaining cash
                      newAgent = executeTrade(newAgent, currentPrice, 'BUY', amountToBuy);
                  }
                  
                  // Simple Profit Taking (Sell if shares > 0 and price is 1% above average buy price)
                  if (newAgent.shares > 0) {
                      const initialBuy = agent.history.find(h => h.type === 'BUY');
                      if (initialBuy && currentPrice >= initialBuy.price * 1.01) {
                          const amountToSell = newAgent.shares * 0.25; // Take 25% profit
                          newAgent = executeTrade(newAgent, currentPrice, 'SELL', amountToSell);
                      }
                  }
              }

              // Update P-Value and Equity
              newAgent.portfolioValue = newAgent.cash + newAgent.shares * currentPrice;
              newAgent.equityCurve.push(newAgent.portfolioValue);
              return newAgent;
          }

          // --- DQN Agent Logic (IDs 1-10) ---

          if (newAgent.candles.length < 21) {
              newAgent.portfolioValue = newAgent.cash + newAgent.shares * currentPrice;
              newAgent.equityCurve.push(newAgent.portfolioValue);
              return newAgent; // Not enough data for indicators
          }

          const inputs = getInputs(newAgent.candles, currentPrice);
          const confidence = newAgent.brain.predict(inputs);
          
          let action = 'WAIT';
          let amount = 0;

          // 3. Stop Loss / Take Profit (Risk Management)
          if (newAgent.shares > 0) {
              const avgBuyTrade = newAgent.history.find(h => h.type === 'BUY' && !h.sold); // Simplified: assumes open position is last Buy
              if (avgBuyTrade) {
                  const buyPrice = avgBuyTrade.price;
                  const lossPercent = (buyPrice - currentPrice) / buyPrice * 100;
                  const profitPercent = (currentPrice - buyPrice) / buyPrice * 100;

                  if (lossPercent >= stopLossPct) {
                      action = 'SELL'; // Force Sell for Stop Loss
                      amount = newAgent.shares;
                      newAgent.logs.push({msg: `STOP LOSS hit at ${stopLossPct}%`, type: 'danger'});
                  } else if (profitPercent >= takeProfitPct) {
                      action = 'SELL'; // Force Sell for Take Profit
                      amount = newAgent.shares;
                      newAgent.logs.push({msg: `TAKE PROFIT hit at ${takeProfitPct}%`, type: 'success'});
                  }
              }
          }
          ...
          if (action === 'WAIT') {
              ...
              } else if (confidence <= 0.35) { // Sell Signal (e.g., < 35% confidence)
                  if (newAgent.shares > 0) {
                      action = 'SELL';
                      amount = newAgent.shares; // Sell all shares
                  }
              }
          }
          ...
          // Execute Trade
          if (action !== 'WAIT') {
              newAgent = executeTrade(newAgent, currentPrice, action, amount);
          } else {
              newAgent.portfolioValue = newAgent.cash + newAgent.shares * currentPrice;
              newAgent.equityCurve.push(newAgent.portfolioValue);
              newAgent.lastAction = 'WAIT';
          }
          
          // 5. Training Step (Reward-based)
          const target = (newAgent.portfolioValue > newAgent.prevValue) ? 1 : 0; // Simple binary reward
          const error = newAgent.brain.train(inputs, target);
          newAgent.loss = error;
  ...
  const handleFileUpload = (e) => {
    ...
    setAssetName(file.name.split('.')[0].toUpperCase()); // Set asset name from filename
    ...
    reader.onload = (event) => {
      ...
      if (parsed.length === 0) {
        alert("Could not parse data. Ensure CSV is formatted correctly.");
        return;
      }
  ...
  const handleExportWeights = (agent) => {
      if (agent.brain) {
          const weights = agent.brain.exportWeights();
          const blob = new Blob([weights], { type: 'application/json' });
          const url = URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.download = `${agent.name}_weights.json`;
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          setAgents(prev => prev.map(a => a.id === agent.id ? {...a, logs: [...a.logs.slice(-49), {msg: "Weights exported.", type:'info'}]} : a));
      }
  };

  // Function to handle import
  const handleImportWeights = (e, agentId) => {
      const file = e.target.files[0];
      if (!file) return;
      ...
      reader.onload = (event) => {
          ...
          setAgents(prev => prev.map(a => {
              if (a.id === agentId && a.brain) {
                  const success = a.brain.importWeights(jsonStr);
                  const logMsg = success ? "Weights imported successfully." : "Error importing weights.";
                  const logType = success ? 'success' : 'danger';
                  return {...a, logs: [...a.logs.slice(-49), {msg: logMsg, type: logType}]};
              }
              return a;
          }));
      };
  ...
  // Function to toggle persistent memory
  const togglePersistentMemory = (agentId) => {
    setAgents(prev => prev.map(a => a.id === agentId ? {
        ...a,
        persistentMemory: !a.persistentMemory,
        logs: [...a.logs.slice(-49), {msg: `Persistent Memory ${a.persistentMemory ? 'Disabled' : 'Enabled'}`, type:'info'}]
    } : a));
  };
  ...
  }, [isRunning, speed, dataSource, dataIndex, customData, startPrice, stopLossPct, takeProfitPct, agents.length]); // Depend on agents.length to ensure correct `updateAgents` closure
  ...
  return (
    <>
              ...
              <div className="sidebar" style={styles.sidebar}>
                <div>
                  ...
                  <div style={{display:'flex', gap:'8px'}}>
                    <button style={{...styles.btn, flex:1, ...(dataSource === 'BINANCE' ? styles.btnPrimary : styles.btnSecondary)}} onClick={() => {setDataSource('BINANCE'); resetSimulation();}}>Live (BTC)</button>
                        ...
                        {agent.brain && (
                          <>
                            <button
                                ...
                                onClick={() => handleExportWeights(agent)}
                            >
                                💾 Export
                            </button>
                            <label style={{...styles.btnSecondary, flex:1, margin:0}}>
                                ⬆️ Import
                                <input 
                                    type="file" 
                                    hidden 
                                    accept=".json" 
                                    onChange={(e) => handleImportWeights(e, agent.id)}
                                />
                            </label>
                            <button
                                style={{...styles.btn, flex:1, backgroundColor: agent.persistentMemory ? THEME.success : '#fff', color: agent.persistentMemory ? '#fff' : '#475569', border: agent.persistentMemory ? 'none' : `1px solid ${THEME.border}`}}
                                onClick={() => togglePersistentMemory(agent.id)}
                                title="Keep trained model weights on Reset"
                            >
                                🧠 {agent.persistentMemory ? 'PERSISTENT' : 'RESET ON END'}
                            </button>
                          </>
                        )}
                      </div>
      ...
      {modelInfoAgent && (
        <ModelInfoModal 
          agent={modelInfoAgent} 
          onClose={() => setModelInfoAgent(null)} 
        />
      )}
    </>
  );
