...
const calculateReward = (agent, action, pnl, currentPrice, allCandles) => {
  ...
  // const scaledPnL = Math.max(-5, Math.min(5, pnlPercent));
  
  // NEW:
  const pnlPercent = (pnl / CONFIG.INITIAL_CASH) * 100;
  const scaledPnL = Math.max(-10, Math.min(10, pnlPercent * 2)); // 2x multiplier, ±10 range

  // ✅ THEN UPDATE ALL REWARD VALUES IN THE SWITCH STATEMENT:
  switch (rewardType) {
    case 'scalper':
      if (action === 2 && pnl > 0) {
        reward = scaledPnL * (holdTime < 10 ? 2.0 : 1.0); // CHANGE: was 1.5
      } else if (action === 2 && pnl < 0) {
        reward = scaledPnL * 1.5;
      } else if (action === 0) {
        reward = -0.1; // CHANGE: was -0.05
      } else if (action === 1 && agent.shares > 0) {
        reward = -0.02 * Math.min(holdTime, 20); // CHANGE: was -0.01
      }
      break;

    case 'trend':
      if (action === 2 && pnl > 0) {
        reward = scaledPnL * (1 + holdTime / 50); // CHANGE: was holdTime / 100
      } else if (action === 2 && pnl < 0) {
        reward = scaledPnL * 1.5; // CHANGE: was 1.2
      } else if (action === 1 && agent.shares > 0) {
        const unrealized = lastBuyTrade ? (currentPrice - lastBuyTrade.price) / lastBuyTrade.price : 0;
        reward = unrealized > 0 ? 0.05 : -0.02; // CHANGE: was 0.02, -0.01
      }
      break;
    ...
    case 'momentum':
      if (action === 0) {
        ...
        if (sma5 && sma20 && sma5 > sma20) {
          ...
          reward = -0.2; // CHANGE: was -0.1
        }
      } else if (action === 2) {
        reward = scaledPnL * 1.5; // CHANGE: was scaledPnL (no multiplier)
      }
      break;
    ...
    case 'mean-reversion':
      if (action === 0) {
        ...
        reward = scaledPnL * 1.5; // CHANGE: was 1.3
      } else if (action === 2 && pnl < 0) {
        reward = scaledPnL * 1.5; // CHANGE: was 1.3
      }
      break;
    ...
    case 'aggressive':
      if (action === 2) {
        reward = scaledPnL * 2.0; // CHANGE: was 1.2
      } else if (action === 0) {
        reward = -0.05; // CHANGE: was -0.02
      }
      break;

    case 'conservative':
      if (action === 2 && pnl < 0) {
        reward = scaledPnL * 3; // CHANGE: was 2
      } else if (action === 2 && pnl > 0) {
        reward = scaledPnL * 1.0; // CHANGE: was 0.8
      } else if (action === 1 && agent.shares > 0) {
        reward = 0.01; // CHANGE: was 0.005
      }
      break;
    ...
    case 'volatility':
      ...
      if (action === 2) {
        reward += scaledPnL * 1.5; // CHANGE: was scaledPnL
      }
      break;
    ...
    case 'contrarian':
      if (action === 0) {
        ...
        const sma20 = calculateSMA(allCandles, 20);
        if (sma5 && sma20 && sma5 < sma20) {
          reward = 0.15; // CHANGE: was 0.08
        }
      } else if (action === 2) {
        reward = scaledPnL * 1.5; // CHANGE: was 1.1
      }
      break;
    ...
    default:
      if (action === 2) {
        ...
        const unrealized = lastBuyTrade ? (currentPrice - lastBuyTrade.price) / lastBuyTrade.price : 0;
        reward = unrealized * 0.05; // CHANGE: was 0.02
      }
      break;
  }

  // ✅ UPDATE FINAL CLIPPING:
  // OLD: return Math.max(-5, Math.min(5, reward));
  // NEW:
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setAgents(prevAgents => prevAgents.map(agent => {
        ...
        // ================================================================
        // AGENT ACTION SELECTION (NO CONSTRAINTS!)
        // ================================================================

        if (agent.id === 11) {
          // Buy & Hold: Buy once at start
          if (!agent.hasBoughtInitial && currentPrice > 0) {
            action = 0; // BUY
          } else {
            action = 1; // HOLD forever
          }
        } else if (agent.id === 12) {
          // Dip-Buyer: Rule-based strategy
          const sma5 = calculateSMA(allCandles, 5);
          ...
          if (!agent.hasBoughtInitial && currentPrice > 0) {
            ...
            action = 1; // Hold
              ...
              // ✅ ADD THIS BLOCK - ML agents need to predict actions!
...
// ACTION 3: SHORT (Open Short Position) - IMPROVED SIZING
  ...
  tradeType = 'SHORT';
        ...
        // ACTION 4: COVER (Close Short Position) - NO BLOCKING!
          ...
          // Calculate total revenue from shorting
          let totalRevenue = 0;
          for (const item of newHistory) {
            if (item.type === 'SHORT') {
          ...
          newCash -= coverCost; // Pay to cover
          newCash += pnl; // Apply profit/loss
          newShortShares = 0;
        ...
        // ✅ NOW train with the completed action
        if (agentModel && previousState) {
        ...
        // Calculate short P&L
        const shortPnL = newShortShares > 0 ? (() => {
          let totalRevenue = 0;
          for (const item of newHistory) {
            if (item.type === 'SHORT') {
              totalRevenue += item.amount * item.price * (1 - CONFIG.COMMISSION);
          ...
          const currentCost = newShortShares * currentPrice / (1 - CONFIG.COMMISSION);
          return totalRevenue - currentCost;
        })() : 0;

        const finalPortfolioValue = newCash + longValue + shortPnL;
