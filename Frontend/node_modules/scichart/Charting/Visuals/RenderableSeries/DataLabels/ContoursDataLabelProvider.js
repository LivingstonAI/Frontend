...
var ContoursDataLabelProvider = /** @class */ (function (_super) {
    ...
    ContoursDataLabelProvider.prototype.generateDataLabels = function (renderContext, renderPassData) {
        ...
        var _a = this.parentSeries.dataSeries, xStart = _a.xStart, xStep = _a.xStep, yStart = _a.yStart, yStep = _a.yStep;
        var contourParams = this.parentSeries.getContourDrawingParams();
        this.makeContourLabels(contourParams, yStart, yStep, renderPassData, xStart, xStep, font, bounds);
    };
    ContoursDataLabelProvider.prototype.makeContourLabels = function (contourParams, yStart, yStep, renderPassData, xStart, xStep, font, bounds) {
        var _a, _b, _c, _d;
        var zStep = contourParams.majorStepZ;
        var offset = contourParams.zOffset < 0 ? zStep + contourParams.zOffset : contourParams.zOffset % zStep;
        ...
        for (var yIndex = 0; yIndex < this.dataSeries.arrayHeight; yIndex++) {
            ...
            for (var xIndex = 0; xIndex < this.dataSeries.arrayWidth; xIndex++) {
                ...
                var text = undefined;
                var lineX = x;
                if (z < contourParams.zMin || z > contourParams.zMax)
                    continue;
                var firstlabel = true;
                if (lastZ !== undefined) {
                    var prevZ = offset + Math.floor((lastZ - offset) / zStep) * zStep;
                    ...
                    if (z <= prevZ) {
                        ...
                        lineX = firstlabel ? x : (xStep * (prevZ - z)) / (lastZ - z) - xStep / 2 + x;
                        firstlabel = false;
                    }
                    else if (z >= nextZ) {
                        ...
                        lineX = firstlabel ? x : (xStep * (nextZ - z)) / (z - lastZ) - xStep / 2 + x;
                        firstlabel = false;
                    }
                    //console.log(lastZ, z, prevZ, text, lineX);
                }
                lastZ = z;
                // else {
                //     lastZ = offset + Math.floor((z - offset) / zStep) * zStep;
                // }
                if (!text) {
                    continue;
                }
