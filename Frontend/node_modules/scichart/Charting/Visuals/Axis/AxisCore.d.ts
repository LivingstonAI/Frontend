...
export declare type TAxisTitleStyle = TTextStyle & {
    ...
    rotation?: number;
    /**
     * Whether to use native text rendering or not. If not specified, defaults to the axis' `useNativeText` setting, which by default is `true`.
     * @note While faster, Native text does NOT have quite the same font and style support. For more flexible title styles, set this to `false`.
     */
 ...
 */
export declare abstract class AxisCore extends DeletableEntity implements IAxisParams, IDeletable, IIncludable {
    /**
     * The type of axis. See {@link EAxisType} for a list of values
     */
    abstract readonly type: EAxisType;
    /**
     * A unique Id for the {@link AxisCore}
     */
    readonly id: string;
    /**
     ...
     */
    abstract animateVisibleRange(visibleRange: NumberRange, durationMs: number, easingFunction: TEasingFn, onCompleted: () => void): IGenericAnimation | undefined;
    /**
     * The MajorDelta is the spacing between major gridlines and axis labels.
     * @remarks
     * This is internally computed via the {@link AxisCore.deltaCalculator | Delta Calculator}, however it can be explicitly set here
