...
var NativeTextAnnotation = /** @class */ (function (_super) {
    ...
    NativeTextAnnotation.prototype.drawWithContext = function (renderContext, xCalc, yCalc, seriesViewRect, surfaceViewRect, chartViewRect) {
        ...
        var getX1Y1 = function () {
            ...
            // further adjust so padding "pushes" the text's positioning, not the other way around
            if (_this.horizontalAnchorPointProperty === AnchorPoint_1.EHorizontalAnchorPoint.Left) {
                x1$ += _this.padding.left;
            }
            else if (_this.horizontalAnchorPointProperty === AnchorPoint_1.EHorizontalAnchorPoint.Right) {
                x1$ -= _this.padding.right;
            }
            else {
                x1$ -= (_this.padding.right - _this.padding.left) / 2;
            }
            if (_this.verticalAnchorPointProperty === AnchorPoint_1.EVerticalAnchorPoint.Top) {
                y1$ += _this.padding.top;
            }
            else if (_this.verticalAnchorPointProperty === AnchorPoint_1.EVerticalAnchorPoint.Bottom) {
                y1$ -= _this.padding.bottom;
            }
            else {
                y1$ += (_this.padding.top - _this.padding.bottom) / 2;
            }
            return { x1: x1$, y1: y1$ };
        };
    ...
    NativeTextAnnotation.prototype.drawBackground = function (renderContext, textWidth, textHeight, rotationCenter, clipRect) {
        var totalWidth = textWidth + this.padding.left + this.padding.right;
        var totalHeight = textHeight + this.padding.top + this.padding.bottom;
        var fromCenterToTop;
        var fromCenterToBottom;
        switch (this.verticalAnchorPoint) {
            case AnchorPoint_1.EVerticalAnchorPoint.Bottom:
                fromCenterToTop = totalHeight;
                fromCenterToBottom = 0;
                break;
            case AnchorPoint_1.EVerticalAnchorPoint.Top:
                fromCenterToTop = 0;
                fromCenterToBottom = totalHeight;
                break;
            default:
                fromCenterToTop = totalHeight / 2;
                fromCenterToBottom = totalHeight / 2;
        }
        ...
        switch (this.horizontalAnchorPoint) {
            case AnchorPoint_1.EHorizontalAnchorPoint.Right:
                fromCenterToLeft = totalWidth;
                fromCenterToRight = 0;
                break;
            case AnchorPoint_1.EHorizontalAnchorPoint.Left:
                fromCenterToLeft = 0;
                fromCenterToRight = totalWidth;
                break;
            default:
                fromCenterToLeft = totalWidth / 2;
                fromCenterToRight = totalWidth / 2;
        }
    ...
    /** @inheritdoc */
    NativeTextAnnotation.prototype.getAnnotationBorders = function (ordered, applyDelta) {
        if (ordered === void 0) { ordered = false; }
        if (applyDelta === void 0) { applyDelta = false; }
        var borders = _super.prototype.getAnnotationBorders.call(this, ordered, applyDelta);
        return {
            x1: borders.x1 - this.padding.left,
            x2: borders.x2 + this.padding.right,
            y1: borders.y1 - this.padding.top,
            y2: borders.y2 + this.padding.bottom
        };
    };
