...
export default function Quizzifier() {
    ...
    const fetchQuizData = async () => {
        ...
        try {
                    ...
                    messages: [
                        {
                            ...
                            Ensure the JSON output is properly formatted and does not include extra explanations.`,
                        },
            ...
            const responseText = await response.text(); // Get raw response as text for better debugging
            console.log("Raw API Response:", responseText);
    
            const data = JSON.parse(responseText); // Parse the response into JSON
            const quizContent = JSON.parse(data.choices[0].message.content); // Parse the quiz content
                                ...
                                <ul style={styles.feedbackList}>
                                    {answers.map((answer, index) => (
                                        <li
                                            key={index}
                                            style={{
                                                color: answer.isCorrect ? "#61ff33" : "#ff5e33",
                                            }}
                                        >
                                            Q: {answer.question}
                                            <br />
