...
export default function CallAI() {
    ...
    const avatarGradients = [
        "linear-gradient(135deg, #4361ee, #3a0ca3)", // Original dark blue
        "linear-gradient(135deg, #065f46, #064e3b)", // Dark green
        "linear-gradient(135deg, #581c87, #6b21a8)", // Dark purple
        "linear-gradient(135deg, #7f1d1d, #991b1b)", // Dark red
        "linear-gradient(135deg, #1e3a8a, #1e40af)"  // Navy blue
    ];
    ...
    // Function to handle avatar click and change color
    const handleAvatarClick = () => {
        if (isCallActive) {
            setAvatarColorIndex((prevIndex) => (prevIndex + 1) % avatarGradients.length);
        }
    };
                        ...
                        <div className={`phone-interface ${isCallActive ? 'active-call' : ''}`}>
                            <div className="caller-display">
                                <div className="caller-avatar">
                                    <div 
                                        className={`avatar-circle ${isCallActive ? 'active' : ''}`}
                                        onClick={handleAvatarClick}
                                        style={{ background: avatarGradients[avatarColorIndex] }}
                                    >
                                        {isCallActive && (
