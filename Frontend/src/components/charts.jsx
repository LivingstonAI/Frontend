...
export default function Charts() {
    ...
    const [monthSeparators, setMonthSeparators] = useState([]);
...
Use this context to provide informed trading insights. Be concise, helpful, and professional.`
    ...
    const fetchTimeSeparators = async (assetInfo, timeframeConfig) => {
        try {
            const symbol = assetInfo.yfinanceSymbol || assetInfo.binanceSymbol || assetInfo.symbol;
            
            const response = await fetch(`${baseUrl}/api/snowai-time-separators/`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!response.ok) {
                throw new Error(`Separator API error: ${response.status}`);
            }
            
            const result = await response.json();
            
            if (result.success) {
                setWeekSeparators(result.week_starts || []);
                setMonthSeparators(result.month_starts || []);
                console.log(`Fetched ${result.week_count} weeks and ${result.month_count} months`);
            }
        } catch (error) {
            console.error('Error fetching time separators:', error);
        }
    };

    const toggleTimeSeparators = async () => {
        if (!showSeparators) {
            const assetInfo = getCurrentAssetInfo();
            const timeframeConfig = timeframes[timeframe];
            await fetchTimeSeparators(assetInfo, timeframeConfig);
        }
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (showSeparators) {
                    await fetchTimeSeparators(assetInfo, timeframes[timeframe]);
                }
    ...
    }, [marketData, chartType, tvLoaded, movingAverages, showVolume, showSeparators, weekSeparators, monthSeparators]);
                ...
                <>
                    <div style={styles.drawingToolbar}>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', flex: 1 }}>
                            ...
                            <button
                                onClick={toggleTimeSeparators}
                                style={{
                                    ...styles.toolButton,
                                    ...(showSeparators ? styles.toolButtonActive : {}),
                                    background: showSeparators ? 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)' : 'white',
                                    color: showSeparators ? 'white' : '#475569'
                                }}
                            >
                                📅 Week/Month Lines
                            </button>
